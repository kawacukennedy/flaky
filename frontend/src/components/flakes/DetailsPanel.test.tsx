...
describe('DetailsPanel', () => {
  const mockSummaryContent = <div data-testid="summary-content">Summary Details</div>;
  const mockRootCausesContent = <div data-testid="root-causes-content">Root Causes Analysis</div>;
  const mockLinkedIssuesContent = <div data-testid="linked-issues-content">Linked Issues List</div>;
  ...
  it('renders the tabs correctly', () => {
    ...
    expect(screen.getByRole('tab', { name: 'Root Causes' })).toBeInTheDocument();
    expect(screen.getByRole('tab', { name: 'Linked Issues' })).toBeInTheDocument();
  });
