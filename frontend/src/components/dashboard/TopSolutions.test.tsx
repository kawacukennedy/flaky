...
describe('TopSolutions', () => {
  ...
  it('renders a list of solution cards when provided', () => {
    const solutions = [
      { id: '1', title: 'Fixing Flaky Login', snippet: 'A quick fix for login issues.', votes: 15 },
      { id: '2', title: 'Optimizing Database Queries', snippet: 'Improve performance with these tips.', votes: 22 },
    ];
    render(<TopSolutions solutions={solutions} />);
    expect(screen.getByText('Fixing Flaky Login')).toBeInTheDocument();
    expect(screen.getByText('A quick fix for login issues.')).toBeInTheDocument();
    ...
    expect(screen.getByText('Improve performance with these tips.')).toBeInTheDocument();
