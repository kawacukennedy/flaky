...
import { fetchFilteredTests, setSelectedTestId } from '../app/slices/testsListSlice';
...
const DashboardPage: React.FC = () => {
  ...
  const stableTests = testsList.data.filter(test => test.flakiness_score <= 0.5);

  // Handle empty datasets
  const pieChartData = testsList.data.length > 0 ? {
  ...
  const lineChartData = testsList.data.length > 0 ? {
    labels: ['Tests'],
    datasets: [
      {
        label: 'Total Tests',
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <SearchBar searchTerm={filters.query} onSearchChange={handleSearchChange} placeholder="Search tests..." />
        <SearchFilter filters={filters.appliedFilters} onFilterChange={handleFilterChange} options={filterOptions} />
